...
const MAX_RETRIES = 3;
const API_URL = 'https://red-rotation-three.vercel.app/api/nowplaying.json?username=ztz0&fallback=true';
...
async function fetchTrackWithRetry(retries = MAX_RETRIES): Promise<Track | null> {
  try {
    ...
    if (!data.track) {
      throw new Error('No track found in response');
    }
...
function renderTrack(container: HTMLElement, track: Track | null) {
  ...
  if (!track) {
    ...
    if (nowPlaying) {
      nowPlaying.textContent = 'No song is currently playing';
    }
  ...
  if (albumArt) {
    albumArt.src = image[2]!['#text'] || 'https://lastfm.freetls.fastly.net/i/u/34s/2a96cbd8b46e442fc41c2b86b821562f.png';
  }
  ...
  if (artistName) {
    artistName.textContent = artist['#text'];
  }

  if (albumName) {
    albumName.textContent = album['#text'];
  }
